...
window.BrainCore = (function () {
  ...
  // ======================= boot =======================
  async function boot(skin, source, root) {
    destroy();
    S.skin = skin;
    S.source = source;
    S.root = root;
  ...
  async function expandAll() {
    ...
    const results = await Promise.all(targets.map(d =>
      S.source.expand(d.id).catch(() => null)));
  ...
  function select(n) {
    ...
    card.querySelectorAll('.act').forEach(b => b.onclick = async () => {
      ...
      if (act === 'edit') { // /tweak principle: pick an item, change it, it sticks
        ...
        await S.source.tweak({ action: 'edit', id: n.id, label: label.trim() || n.label, desc: desc === null ? n.desc : desc });
      ...
      if (act === 'remove') { // /tweak principle: delete items you don't want on the map
        ...
        await S.source.tweak({ action: 'hide', id: n.id });
  ...
  async function apiOpen(path) {
    if (!path) return;
    try {
      const d = await S.source.open(path);
  ...
  async function openViewer(path) {
    ...
    try {
      const d = await S.source.file(path);
  ...
  function buildPanels() {
    const skin = S.skin;
    // HUD (top-left): brand block
    const hexSvg = `<svg viewBox="0 0 48 48" width="30" height="30" fill="none" stroke="#4f6bff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M24 6l18 10v16L24 42 6 32V16Z"/><path d="M24 6v16l18 10" opacity="0.5"/><path d="M24 22L6 32" opacity="0.5"/></svg>`;
    document.getElementById('brain-hud').innerHTML = `
      <div class="hud-brand">${hexSvg}<span class="hud-name">JARVIS</span><span class="hud-product">CORTEX</span></div>
    ...
    document.querySelectorAll('#tweak-body input[type=range]').forEach(inp => {
      inp.oninput = () => {
        ...
        if ((k.startsWith('grav_') || k.startsWith('dist_')) && S.simKind) { clearTimeout(S._gravT);
    clearInterval(S.controlsInterval); S._gravT = setTimeout(() => refreshLayout(0.35), 160); }
      };
    });
    ...
    document.getElementById('btn-bake').onclick = async () => {
      ...
      try {
        const d = await S.source.bake(bake);
    ...
    async function refreshData(msg) {
      ...
      try {
        await S.source.rescan();
        const data = await S.source.graph();
    ...
    document.getElementById('tweak-restore').onclick = async () => {
      await S.source.tweak({ action: 'unhide-all' });
    ...
    input.oninput = () => {
      ...
      debounce = setTimeout(async () => {
        ...
        const d = await S.source.search(q);
  ...
  function applyColors() {
    ...
    if (S.T) {
      ...
      S.root.style.setProperty('--bg', o.bg || (base.css && base.css.bg));
      S.root.style.setProperty('--accent', S.accent);
    }
  ...
  function destroy() {
    ...
    if (S.sim) S.sim.stop();
    clearTimeout(toastTimer);
    clearTimeout(S._gravT);
    if (S.root) S.root.replaceChildren();
    S.root?.classList.remove('menu-open', 'lg-open');
  }

  return { boot, destroy, S, expandDir, collapseDir, jumpTo, openViewer, apiOpen, toast, flyToNode, select };
})();
